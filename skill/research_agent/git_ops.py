...
# Extensions and directories that should NEVER be tracked by git.
# These are data, weights, and experiment artifacts — not code.
IGNORE_PATTERNS = [
    # Model weights & checkpoints
    "*.pth", "*.pt", "*.ckpt", "*.safetensors", "*.bin", "*.onnx",
    # Data files
    "*.nii", "*.nii.gz", "*.npy", "*.npz", "*.h5", "*.hdf5",
    "*.pkl", "*.pickle", "*.parquet", "*.arrow", "*.tfrecord",
    "*.dcm", "*.mha", "*.nrrd",
    # Large archives
    "*.tar", "*.tar.gz", "*.tgz", "*.zip",
    # Common data/checkpoint directories
    "checkpoints/", "nnUNet_raw/", "nnUNet_preprocessed/", "nnUNet_results/",
    "data/", "weights/", "pretrained/", "test_predictions/",
    # Experiment logs and outputs (tracked via state.json, not git)
    "logs/", "wandb/", "mlruns/", "tensorboard/",
    # Python / OS
    "__pycache__/", "*.pyc", ".DS_Store",
    # Research agent internal
    "results/", "state.json", "progress.md",
]

# Only stage files matching these extensions (code + config + docs).
CODE_EXTENSIONS = {
    ".py", ".sh", ".yaml", ".yml", ".toml", ".cfg", ".ini", ".conf",
    ".json",  # small config files only — large ones blocked by size check
    ".md", ".rst", ".txt",
    ".dockerfile", ".Dockerfile",
    ".gitignore", ".gitattributes",
}

# Also stage these exact filenames regardless of extension.
CODE_FILENAMES = {
    "Makefile", "Dockerfile", "Procfile", "requirements.txt",
    "setup.py", "setup.cfg", "pyproject.toml",
}

# Maximum file size to stage (bytes). Anything larger is likely data, not code.
...
def _ensure_gitignore() -> None:
    ...
    If .gitignore doesn't exist, create it. If it exists, append any
    missing patterns. This prevents data/weights from ever being staged.
    """
    ...
    with open(gitignore, "a") as f:
        if existing:
            f.write("\n")
        f.write("# Auto-generated by research_agent — protect data/weights from git\n")
        for p in missing:
            f.write(f"{p}\n")

    print(f"Updated .gitignore: added {len(missing)} pattern(s)", file=sys.stderr)


def _is_code_file(path: str) -> bool:
    """Check if a file looks like code (not data/weights)."""
    p = Path(path)

    # Always allow exact filenames
    if p.name in CODE_FILENAMES:
        return True

    # Check extension
    # Handle compound extensions like .nii.gz
    if p.name.endswith(".nii.gz") or p.name.endswith(".tar.gz"):
        return False

    if p.suffix.lower() not in CODE_EXTENSIONS:
        return False

    # Check file size — large files are likely data even with code extensions
    try:
        if p.exists() and p.stat().st_size > MAX_FILE_SIZE:
            print(f"  SKIP (>1MB): {path}", file=sys.stderr)
            return False
    except OSError:
        pass
...
def _stage_code_only() -> list[str]:
    """Stage only code files. Returns list of staged file paths.

    Uses `git status --porcelain` to find changed/new files, then
    filters to code-only files before staging. Prints skipped files
    so the user knows what was excluded.
    """
    _ensure_gitignore()

    # First, stage the .gitignore itself if it changed
    _run(["git", "add", ".gitignore"], check=False)
    ...
    for line in r.stdout.strip().splitlines():
        ...
        if status.strip() == "D":
            # Deleted files — always stage the deletion
            _run(["git", "add", filepath], check=False)
            staged.append(filepath)
        elif _is_code_file(filepath):
            _run(["git", "add", filepath], check=False)
            staged.append(filepath)
        else:
    ...
    if skipped:
        print(f"Skipped {len(skipped)} non-code file(s):", file=sys.stderr)
        for f in skipped[:10]:
    ...
    if staged:
        print(f"Staged {len(staged)} code file(s):", file=sys.stderr)
        for f in staged[:10]:
            print(f"  ADD: {f}", file=sys.stderr)
        if len(staged) > 10:
            print(f"  ... and {len(staged) - 10} more", file=sys.stderr)
...
def cmd_branch_start(args) -> None:
    ...
    # Check if we're on main — warn if not
    current = _current_branch()
    if current != "main":
        has_uncommitted = _has_changes()
        # Check if current branch has commits not on main
        r = _run(["git", "log", f"main..{current}", "--oneline"], check=False)
        unmerged_commits = [l for l in r.stdout.strip().splitlines() if l.strip()]

        warnings = []
        if has_uncommitted:
            warnings.append(f"  - Uncommitted changes on {current}")
        if unmerged_commits:
            warnings.append(f"  - {len(unmerged_commits)} commit(s) on {current} not merged to main")

        if warnings:
            print(f"WARNING: Currently on branch '{current}', not main.", file=sys.stderr)
            for w in warnings:
                print(w, file=sys.stderr)
            print(f"ACTION_NEEDED: Ask user whether to merge '{current}' to main first, "
                  f"or discard and continue.", file=sys.stderr)
            if not args.force:
                sys.exit(2)
...
def cmd_commit_code(args) -> None:
    ...
    Only stages code files (scripts, configs, docs). Data, weights,
    checkpoints, and other large files are never staged.
    """
    if not _has_changes():
    ...
    # Stage only code files — never data/weights
    staged = _stage_code_only()
    if not staged:
        print("No code files to commit (only data/weight files changed).", file=sys.stderr)
        sys.exit(1)
...
def cmd_commit_results(args) -> None:
    ...
    if staged:
        ...
        # No code files changed — commit empty with message for the record
        _run(["git", "commit", "--allow-empty", "-m", msg])
        print(f"Committed results (no code changes) for iteration {args.iteration}")
...
def main():
    ...
    p_bs.add_argument("--force", action="store_true",
                       help="Skip warning and switch to main even if current branch has unmerged work")
